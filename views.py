...
class GenTokensBtn(discord.ui.Button):
    ...
    async def callback(self, interaction):
        await interaction.response.edit_message(
            content="Select the action to enable with this token",
            view=GenTokenSelectOperations(),
        )
...
class GenPasswordTokenBtn(discord.ui.Button):
    def __init__(self):
        super().__init__(
            label="Create a password token", style=discord.ButtonStyle.blurple
        )

    @app_commands.checks.has_permissions(administrator=True)
    async def callback(self, interaction):
        await interaction.response.edit_message(
            content="Select the action to enable with this password",
            view=GenPasswordSelectOperations(),
        )
    ...
    async def callback(self, interaction):
        ...
        async def select_roles_callback(interact):
            ...
            await interact.response.edit_message(
                content=gen_text_description_actions(self.data),
                view=self.class_to_use(self.data),
            )

        select_roles.callback = select_roles_callback
        await interaction.response.edit_message(
            content="Select the role to add with tokens", view=view
        )
...
class RenameBtn(discord.ui.Button):
    ...
    async def callback(self, interaction):
        async def next_action(interact):
            self.data["rename"] = interact.data["components"][0]["components"][0][
                "value"
            ]
            await interact.response.edit_message(
                content=gen_text_description_actions(self.data),
        ...
        await asyncio.gather(
            ...
            interaction.edit_original_response(
                content="Insert the name to rename", view=OpenModalView(modal)
            ),
        )
...
class GenPasswordSelectOperations(discord.ui.View):
    def __init__(self, data=None):
        ...
        self.data = data if data else {}
        self.add_item(
            SelectRoleBtn(self.data, class_to_use=GenPasswordSelectOperations)
        )
        ...
        async def nextop(interaction):
            ...
            await asyncio.gather(
                interaction.response.send_modal(modal),
                interaction.edit_original_response(
                    content="Insert the password to use", view=OpenModalView(modal)
                ),
            )
    ...
    def __init__(self, data=None):
        ...
        async def nextop(interaction):
            ...
            await asyncio.gather(
                interaction.response.send_modal(modal),
                interaction.edit_original_response(
                    content="Insert the amount of tokens to generate",
                    view=OpenModalView(modal),
                ),
            )
    ...
    async def callback(self, interaction):
        ...
        async def select_roles_callback(interaction):
            ...
            await interaction.edit_original_response(
                content="Insert the infromation about the emails",
                view=EmailDetailsView(self.data),
            )
