...
EMAIL_FORMAT_REGEX = re.compile(
    "(?P<email>" + EMAIL_REGEX + ")(:?[ ]*<(?P<nickname>.*)>)?"
)
...
async def action_handler(action, interaction):
    if action.type == "role":
        ...
        try:
            roles = [
                discord.utils.get(interaction.guild.roles, id=int(role))
        ...
        return (
            "Authenticated successfully with role(s) '"
...
async def _send_email_wih_token(email, object, message, roles, guild_id, client=None):
    try:
        ...
        token = await add_token(
            guild_id, roles=roles, email=email["email"], rename=email["nickname"]
        )
...
async def send_emails_with_token(emails, object, message, roles, guild_id):
    ...
    result = await asyncio.gather(
        *[
            _send_email_wih_token(email, object, message, roles, guild_id, client)
            for email in emails
        ]
    )
    try:
    ...
    return [
        ...
        if isinstance(result[i], Exception) or result[i] is None
    ]
